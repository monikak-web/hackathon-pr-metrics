...
export function MedianAuthorLeaderboard({ data }: { data: PrMetric[] }) {
  ...
  useEffect(() => {
    ...
    const grouped = d3.rollup(
      data,
      (v) => d3.median(v, (d) => d.duration_ms!)! / msPerDay,
      (d) => d.author,
    );

    const entries = Array.from(grouped, ([author, medianDays]) => ({
      author,
      medianDays,
    ...
    const x = d3
      .scaleLinear()
      .domain([0, d3.max(entries, (d) => d.medianDays)!])
      .nice()
      .range([0, width]);
    ...
    const color = d3
      ...
      .domain([d3.max(entries, (d) => d.medianDays)!, 0]);
    ...
    g.selectAll("rect")
      ...
      .attr("x", 0)
      .attr("y", (d) => y(d.author)!)
      .attr("width", (d) => x(d.medianDays))
      .attr("height", y.bandwidth())
      .attr("fill", (d) => color(d.medianDays))
      .attr("rx", 4);
    ...
    g.selectAll(".value-label")
      .data(entries)
      .join("text")
      .attr("x", (d) => x(d.medianDays) + 4)
      .attr("y", (d) => y(d.author)! + y.bandwidth() / 2)
      .attr("dominant-baseline", "central")
      .attr("fill", "#a1a1aa")
      .style("font-size", "10px")
      .text((d) => `${d.medianDays.toFixed(1)}d`);
